...
<%@ page language="java" import="java.util.*,Data.Common.*"%>
...
<SCRIPT language=javascript>
        ...
        function add()
        {
        	 Form1.action = "dept.do?action=add";
        	 //Form1.submit();
        }
...
<BODY>
<FORM id=Form1 method=post name=Form1 action=dept.do>
<SCRIPT type=text/javascript>
<!--
...
<TABLE class=tabbar1 border=0 cellSpacing=0 cellPadding=0 align=center>
  ...
  <TR height=24>
    <TD><IMG class=icon src="MyProposerIndex_files/bullet0.gif" height=10> 
      ...
      class=arrow_subtitle>&gt;</SPAN>部门管理 </TD>
...
width="100%" align=center>
  ...
  <TR>
  <!--
  <TD class=title height=24><SPAN id=lblBoeNum>部门名称<font color="#ff0000">*</font></SPAN></TD>
  <TD><SELECT size="0" style="WIDTH: 180px" id=instituteID name=instituteID> 
          <option value="1">武汉研究所</option>
          <option value="2">经营办公室</option>
      </SELECT></TD>
      -->
    <TD class=title height=24><SPAN id=lblBoeNum>部门名称<font color="#ff0000">*</font></SPAN></TD>
    <TD><INPUT style="WIDTH: 172px; HEIGHT: 21px" id=txtDeptName 
    name=txtDeptName></TD>
    <TD class=title><SPAN id=lblDept>部门ID<font color="#ff0000">*</font></SPAN></TD>
    <TD><INPUT style="WIDTH: 128px" id=txtDeptID size=14 
      name=txtDeptID></TD>
    </TR>
  <TR>
...
bgColor=#ffffff>
  ...
  <TR>
    <TD class=tb_titlebar colSpan=6 align=left><IMG 
      ...
      id=lblApprovalList>部门列表</SPAN> </TD></TR></TBODY></TABLE>
<DIV style="WIDTH: 98%" class=autoOver2 align=center>
      ...
      border=1 rules=all cellSpacing=0>
        ...
        <TR class=head id=tr_Head>
           <!--<TD style="WIDTH: 25%" noWrap align=left>部门名称</TD>-->
          <TD style="WIDTH: 25%" noWrap align=left>部门名称</TD>
          <TD style="WIDTH: 25%" noWrap align=left>部门ID</TD>
          <TD noWrap align=left><A 
            href="javascript:__doPostBack('dgApproval$_ctl1$_ctl9','')"></A></TD></TR>
         <%
            HashMap<Integer, DDept> depts = (HashMap<Integer, DDept>)request.getAttribute("detpInfo");
        ...
        <TR class=item>
          ...
          <%
            String institute[] = {"武汉研究所","经营办公室"};
            String instituteType = institute[depts.get(key).getInstitute()-1];
          %>
          <%= instituteType %>
          </TD>
            -->
          <TD noWrap align=left>
          	<%= depts.get(key).getName()%>
          </TD>
          <TD noWrap align=left><SPAN 
            id=dgApproval__ctl2_lbl_DeptName><%= key%></SPAN> </TD>
          <TD noWrap style="text-align: center"><a href="dept.do?action=delete&&id=<%=key%>">删除</a>
