...
<%@ page language="java" import="java.util.*,Business.UserDetail.BUserDetail,Business.Claims.*,Common.*,Common.Location.*,Data.ApprovalInfo.*,Data.UserInfo.*"%>
<HTML>
    <HEAD>
        <title>..................</title>
    ...
    <body>
        <form name="Form1" method="post" action="invoiceDetailGM.do">
    ...
    <tbody>
        <tr height="24">
            <td>
                ...
                <span id="TableHeadBar1_lblHead">当前位置<SPAN class="11air">:</SPAN>与我有关<span class="arrow_subtitle">&gt;</span>单据审批<span class="arrow_subtitle">&gt;</span>总经理审批</span>
            </td>
            <td align="right" width="16" nowrap>
                <img src="BoeWizardIndex_files/icon_collapseall.gif" id="TableHeadBar1_BarImg" class="icon" height="16" style="cursor: hand" data="1" onclick="javascript:ExpandDiv(this);" />
            </td>
        </tr>
    </tbody>
</table>
           ...
           { 
             ...
             if(appCommentIsNull())
              ...
              if(appFlag==1)
              {
               ...
               Form1.action = "generalManager.do?action=permitone"; 
               Form1.submit();
               }                     
          }
          ...
          function notPermit()                         
           { 
             ...
             if(appCommentIsNull())
              ...
              {
               window.top.needAlert = true;
               Form1.action = "generalManager.do?action=notPermitone"; 
               Form1.submit();
               }                     
          }
    ...
    <TD align=left>
      ...
      class=datatbl border=1 rules=all cellSpacing=0>
        ...
        <TR class=head>
          ...
          <TD noWrap align=left style="WIDTH: 15%">审批领导</TD>
          <TD noWrap align=left style="WIDTH: 20%">审批职务</TD>
          <TD noWrap align=left style="WIDTH: 20%">审批时间</TD>
          <TD noWrap align=left style="WIDTH: 15%">审批状态</TD>
          <TD noWrap align=left style="WIDTH: 25%">审批意见</TD></TR>
            <%     
        ...
        <TR class=item>
          ...
          <TD noWrap style="WIDTH: 15%"><SPAN 
            id=dgActive__ctl2_lbl_ApprovalName><%= dApprovalInfo.getApprovalName() %><%= dApprovalInfo.getApprovalId() %></SPAN> </TD>
          <TD noWrap style="WIDTH: 20%"><SPAN 
            id=dgActive__ctl2_lbl_PosElementName><%= roleInfo.get(userInfoNo.get(dApprovalInfo.getApprovalId()).getRoleId()) %></SPAN> </TD>
          <TD noWrap style="WIDTH: 20%"><SPAN id=dgActive__ctl2_lbl_ActiveDate><%= dApprovalInfo.getAppDate() %></SPAN> </TD>
          <TD noWrap style="WIDTH: 15%">          
          <SPAN id=appStatus>
          <%
          String statusStr[] = {"草稿","待项目经理审批","待所长审批", "待财务审批", "正常关闭", "已被退回", "已撤销", "待总经理审批"};
          String appStatus = statusStr[dApprovalInfo.getStatus()];
          %>
          <%= appStatus %>
          </SPAN></TD>          
          <TD style="WIDTH: 25%"><SPAN id=dgActive__ctl2_lbl_Note><%= dApprovalInfo.getComment() %></SPAN> </TD></TR>
          <%   
          i++;
          }
          %> 
...
align=center>
  ...
  <TR>
    ...
    <TD align=left>
       <textarea id="comment" name="comment" cols="165" rows="5">
</textarea>
</TD></TR></TBODY></TABLE> 
...
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" align=center>
 ...
 <TR>         
    <TD class=bottom colSpan=6>
      <DIV align=center> <INPUT style="WIDTH: 50px; HEIGHT: 30px" id=btnQuery class=stbtm01 value=批准  onclick="permit()" type=button name=btnQuery>
       <INPUT style="WIDTH: 50px; HEIGHT: 30px" id=btnQuery  class=stbtm01 onclick="notPermit()" value=退回 type=button name=btnQuery></DIV></TD></TR></TBODY></TABLE>
