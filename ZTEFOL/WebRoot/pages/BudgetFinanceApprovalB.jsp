...
<SCRIPT language=javascript>		
  function query()
        {
	     Form1.action = "budgetFinanceServletB.do?action=query";
	     Form1.submit();
        }
    ...
    function permit()
        {
       		 if(confirm("您确认要提交审批吗?")){
	     			Form1.action = "budgetFinanceServletB.do?action=permit";
	     			Form1.submit();
	    		 }
        }
...
action="budgetFinanceServletB.do">
  ...
  %>
        ...
        <TR id=tr_Head class=head>
          ...
          <TD noWrap style="WIDTH: 5%" align=left>
          	<a href="budgetFinanceServlet.do?action=query&budgetId&userName&project=<%= projectMap.get(projects.get(i))%>">
