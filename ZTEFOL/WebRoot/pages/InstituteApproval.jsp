...
<SCRIPT language=javascript>			
		   ...
		   function query()
                        {
        	            Form1.action = "instituteApproval.do?action=query";
        	            Form1.submit();
                        }
  	  ...
  	  %>
  	    <TR  class=item>
            <TD noWrap align=left>
          	<a href="budgetDetail.do?budgetId=<%= budgetInfos.get(i).getBudgetId()%>&name=<%= budgetInfos.get(i).getName() + budgetInfos.get(i).getNo()%>&dept=<%= budgetInfos.get(i).getDept()%>&submitDate=<%= budgetInfos.get(i).getSubmitDate()%>">
