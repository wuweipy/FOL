...
(function(window, document, undefined) {
    ...
    var defaults = {
        messages: {
            ...
            idefault: 'The %s field is still set to default, please change.',
            ...
            date: '%s格式不对(如：2013-07-01或2013-7-1)'
        },
    ...
    var ruleRegex = /^(.+?)\[(.+)\]$/,
        ...
        dateRegex = /^20\d\d\-[0-1]?\d-(([0-2]?\d)|3[0-1])$/
    ...
    FormValidator.prototype._hooks = {
        ...
        idefault: function(field, defaultName){
        ...
        date: function(field,type) {
        	return (dateRegex.test(field.value));
        },
        
        date_greater: function(field1,matchName){
        	try
        	{
        		 var f1 = splitToInt(field1.value,"-");
             var f2 = splitToInt(this.form[matchName].value,"-");
             var r = (f1[0] - f2[0]) * 10000 + (f1[1] - f2[1]) * 100 + (f1[2] - f2[2]);
             return r >= 0;  
        	}
        	catch(e)
        	{
        ...
        function splitToInt(value,splitStr)
        {
        	 var rtn = value.split(splitStr);
        	 for(var i = 0, length = rtn.length; i < length; i++)
        	 {  
        	       if(rtn[i].charAt(0) == '0')
        	       {
        	       	  rtn[i] = parseInt(rtn[i].charAt(1));
        	       }	
