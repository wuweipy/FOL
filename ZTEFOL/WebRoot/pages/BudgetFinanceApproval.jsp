...
<%@ page language="java" import="java.util.*,Data.Common.Countersign.*,Data.UserInfo.*,Data.Project.*,Data.Common.*,View.ParameterUtil,Business.Budget.*"%>
...
<SCRIPT language=javascript>			
			function clear_text()
			{
				...
				document.all("project").value="";	
				return false;
			}
			
		   function query()
                        {
        	            Form1.action = "budgetFinanceServlet.do?action=query";
        	            Form1.submit();
                        }
...
<INPUT id=projectIN size=1 type=hidden name=projectIN value=<%=ParameterUtil.getChineseString(request, "project", "utf-8")%>>
...
width="100%" align=center>
  ...
  <TR>   
    ...
    <TD class=title>
      <DIV align=left noWrap><SPAN id=departmentxx>项目</SPAN></DIV></TD>
    <TD><SELECT size="0" style="WIDTH: 180px" id=project name=project>    
        <OPTION value=""></OPTION>
    <%    
         ArrayList<DProject> projectList = (ArrayList<DProject>)request.getAttribute("projectInfo");
  	  ...
  	  %>
  	    <TR class=item>
            <TD noWrap align=left>
          	<a href="budgetFinanceDetailServlet.do?budgetId=<%= budgetInfos.get(i).getBudgetId()%>&name=<%= budgetInfos.get(i).getName() + budgetInfos.get(i).getNo()%>&dept=<%= budgetInfos.get(i).getProjectName()%>&submitDate=<%= budgetInfos.get(i).getSubmitDate()%>">
            ...
            <TD noWrap align=left>
            <%= budgetInfos.get(i).getProjectName()%>
            </TD>
            <TD noWrap align=left>
...
function init()
{
   ...
   var projectIN = document.getElementById("projectIN").value;
   if(projectIN == -1 || projectIN == "" || projectIN == "null")
   {
       document.getElementById("project").value = "";
   }
   else
   {
       document.getElementById("project").value = projectIN;
   }   
